#' Plot a histogram of indifference region area from grid search
#' 
#' Using the labeled rank aggregations from the grid search, [rank_aggregation_grid], 
...
plot_histogram_grid <- function(Lambda,orientation='horizontal',min_threshold=0,axis_labels=TRUE,leg.pos='none') {
  Areadf <- Lambda %>% dplyr::count(Rank.Label) #%>% mutate(count = n())
  totalN = sum(Areadf$n)
  ...
  if(orientation=='horizontal') {
    g <- g + 
      ggplot2::geom_col(data=subdf, ggplot2::aes(x=Percent,y=reorder(Rank.Label,n),fill=Rank.Label)) +
      ggplot2::geom_text(data=subdf, ggplot2::aes(x=Percent+1,y=reorder(Rank.Label,n),label=paste0(Percent,"%")),size=3,hjust=0) +
  ...
  if(orientation=='vertical') {
    g <- g + 
      ggplot2::geom_bar(data=subdf, ggplot2::aes(x=reorder(Rank.Label,-n),y=Percent,fill=Rank.Label),stat="identity") +
      ggplot2::geom_text(data=subdf, ggplot2::aes(x=reorder(Rank.Label,-n),y=Percent+1,label=paste0(Percent,"%")),size=3) +
